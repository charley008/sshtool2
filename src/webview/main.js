...
const ConnectionPage = defineComponent({
  ...
  setup(props) {
    ...
    watch(() => props.kind, (newKind) => {
      ...
      Object.keys(info).forEach((key) => { if (!(key in defaults)) delete info[key]; });
      Object.keys(defaults).forEach((key) => { info[key] = defaults[key]; });
    });
    const disposers = [
      bus.on("add", (payload) => {
        mode.value = "add";
...
const ManagerPage = defineComponent({
  ...
  setup(props) {
    ...
    watch(() => draft.forward?.type, (type) => {
      if (type === 2) { draft.forward.remoteHost = ''; draft.forward.remotePort = null; }
    });
    onBeforeUnmount(() => disposers.forEach((dispose) => dispose()));
    ...
    const emitRow = (event, row = draft) => {
      if (event === 'insert') { closeForm(); bus.emit(event, clone(row)); }
      else if (event === 'new') { openForm('new'); }
      else if (event === 'edit') { openForm('edit'); bus.emit('load'); }
      else { bus.emit(event, clone(row)); }
    };
    const editRow = (row) => {
      Object.keys(draft).forEach(k => delete draft[k]);
      Object.assign(draft, clone(row));
      editing.value = true;
      showForm.value = true;
    };
        ...
        <el-table :data="rows" empty-text="暂无数据" size="mini">
          ...
          <template v-if="kind === 'forward'">
            <el-table-column label="本地主机" min-width="100">
              <template #default="{ row }">{{ row.forward?.localHost || '-' }}</template>
            </el-table-column>
            <el-table-column label="本地端口" min-width="80">
              <template #default="{ row }">{{ row.forward?.localPort || '-' }}</template>
            </el-table-column>
            <el-table-column label="目标主机" min-width="100">
              <template #default="{ row }">{{ row.forward?.type === 2 ? 'Not Use' : (row.forward?.remoteHost || '-') }}</template>
            </el-table-column>
            <el-table-column label="目标端口" min-width="80">
              <template #default="{ row }">{{ row.forward?.type === 2 ? 'Not Use' : (row.forward?.remotePort || '-') }}</template>
            </el-table-column>
            <el-table-column label="类型" min-width="90">
              <template #default="{ row }"><el-tag size="mini" type="info" effect="dark">{{ {0:'本地转发',1:'远程转发',2:'Socks5代理'}[row.forward?.type] || '-' }}</el-tag></template>
            </el-table-column>
          </template>
          <template v-if="kind === 'remote'">
            <el-table-column label="分辨率" min-width="100">
              <template #default="{ row }">{{ row.rdp?.desktopGeometry || '-' }}</template>
            </el-table-column>
            <el-table-column label="端口" min-width="70">
              <template #default="{ row }">{{ row.rdp?.port || '-' }}</template>
            </el-table-column>
            <el-table-column label="运行方式" min-width="80">
              <template #default="{ row }">{{ {0:'RDP',1:'VNC'}[row.mode] || 'RDP' }}</template>
            </el-table-column>
          </template>
